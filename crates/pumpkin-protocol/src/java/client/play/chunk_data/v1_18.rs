...
pub fn write_chunk_data(
...

    {
        ...
        write.write_bitset_for_version(&BitSet(Box::new([sky_light_mask as i64])), version)?;
        // Write Block Light Mask
        write.write_bitset_for_version(&BitSet(Box::new([block_light_mask as i64])), version)?;
        // Write Empty Sky Light Mask
        write.write_bitset_for_version(&BitSet(Box::new([sky_light_empty_mask as i64])), version)?;
        // Write Empty Block Light Mask
        write.write_bitset_for_version(&BitSet(Box::new([block_light_empty_mask as i64])), version)?;
