...
impl SalmonVariant {
    ...
    /// Vanilla Salmon spawn weighting: Small = 30, Medium = 50, Large = 15
    #[must_use]
...
/// Represents a Salmon, a schooling fish with three size variants (small, medium, large).
///
/// Vanilla source: `net.minecraft.world.entity.animal.fish.Salmon`
...
/// - Hurt sound on damage (Sound::EntitySalmonHurt)
/// - Size variants: Small, Medium, Large (synced via DATA_TYPE)
/// - Bucket pickup with water bucket -> gives salmon_bucket + plays Sound::ItemBucketFillFish
...
impl SalmonEntity {
    pub fn new(entity: Entity) -> Arc<Self> {
        ...
        let variant = SalmonVariant::random_variant();
        let salmon = Self {
            mob_entity,
    ...
    pub fn set_from_bucket(&self, from_bucket: bool) {
        ...
        entity.set_synced_data(pumpkin_data::tracked_data::salmon::FROM_BUCKET, from_bucket);
    }

    #[must_use]
    pub fn get_variant(&self) -> SalmonVariant {
        SalmonVariant::from_id(self.variant.load(Ordering::Relaxed))
    }
...
impl Mob for SalmonEntity {
    ...
    fn mob_read_nbt(&self, nbt: &NbtCompound) {
        ...
        if let Some(name) = nbt.get_string("type") {
            self.set_variant(SalmonVariant::from_name(&name));
        }
    }
    ...
    fn mob_init_data_tracker(&self) {
        let entity = self.get_entity();
        entity.set_synced_data(
            pumpkin_data::tracked_data::salmon::FROM_BUCKET,
            self.is_from_bucket(),
        );
        entity.set_synced_data(
            pumpkin_data::tracked_data::salmon::DATA_TYPE,
    ...
    fn mob_interact(&self, player: &Arc<Player>, item_stack: &mut ItemStack) -> bool {
        ...
        if item_stack.get_item() == &Item::WATER_BUCKET {
            ...
            if let Some(server) = world.server.upgrade() {
                let mut event =
                    crate::plugin::api::events::player::player_bucket_entity::PlayerBucketEntityEvent {
                        ...
                        bucket_item: "salmon_bucket".to_string(),
                        cancelled: false,
                    };
            ...
            let mut bucket = ItemStack::new(1, &Item::SALMON_BUCKET);
    ...
    fn mob_set_variant_name(&self, name: &str) {
        ...
        self.set_variant(SalmonVariant::from_name(clean));
    }
}
